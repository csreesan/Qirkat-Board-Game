...
class Move {
    ...
    /** Patter for valid square input for vestigial move. */
    private static final Pattern SQ_PATTERN =
    ...
    static Move move(Move move0, Move move1) {
        ...
        if (move0.jumpTail() == null) {
            if (move0.toIndex() != move1.fromIndex()) {
                if (move1.isVestigial()) {
                    return move(move0, move(move0.col1(), move0.row1(),
                            move1.col0(), move1.row0()));
                }
                throw new IllegalArgumentException("Moves not connected");
            }
            return move(move0.col0(), move0.row0(), move0.col1(), move0.row1(),
                    move(move0.col1(), move0.row1(), move1.col1(), move1.row1(),
                            move1.jumpTail()));
        } else {
            return move(move0.col0(), move0.row0(), move0.col1(), move0.row1(),
     ...
     *  the left. */
    boolean isLeftMove() {
        return (_col1 - _col0) < 0 && _row0 == _row1;
    }
    ...
    /** Return the vestigial Move denoted by SQR. */
    static Move parseVestMove(String sqr) {
        Matcher mat = SQ_PATTERN.matcher(sqr);
        if (!mat.matches()) {
            throw new IllegalArgumentException("bad move denotation");
        }
        return move(mat.group().charAt(0), mat.group().charAt(1));
    ...
    private void toString(Formatter out) {
        ...
        Move next = jumpTail();
        while (next != null) {
            out.format("-%c%c", next.col1(), next.row1());
            next = next.jumpTail();
        }
    }
